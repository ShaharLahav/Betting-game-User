...
export class SchedulePage extends MeteorComponent {
  ...
  updateSchedule() {
    ...
    if (this.segment === 'all') {
      ...
      // let mili = time.getTime();
      console.log(mili);
      this.gamess = Games.find({ "time": { "$gt": mili } });
      console.log(this.gamess.fetch());
      console.log(Games.find({}).fetch());

    }
    else
      if (this.segment === 'favorites')
        this.gamess = Games.find({ "time": { "$lte": mili } });
